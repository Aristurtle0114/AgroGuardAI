...
export const analyzeCropImage = async (base64Image: string): Promise<AIDetectionResponse> => {
  try {
    ...
    throw new Error('Analysis failed. Check your connection or try a clearer image.');
  }
};
...
export const chatWithExpert = async (history: ChatMessage[], message: string) => {
  try {
    const chat = ai.chats.create({
      model: 'gemini-3-flash-preview',
      config: {
        systemInstruction: 'You are an Expert Agronomist AI. Provide helpful, accurate advice to farmers regarding crop health, pest control, and soil management. Always recommend organic or biological solutions where effective.',
        tools: [{ googleSearch: {} }],
      },
    });
    const response = await chat.sendMessage({ message });
    const text = response.text || "I am currently processing that. Could you please rephrase?";
    const grounding = response.candidates?.[0]?.groundingMetadata?.groundingChunks || [];
    const links = grounding.filter(c => c.web).map(c => ({ title: c.web?.title, uri: c.web?.uri }));
    return { text, links };
  } catch (error) {
    console.error('Expert Chat Error:', error);
    throw new Error('Could not connect to the Agronomist AI. Please try again later.');
  }
};